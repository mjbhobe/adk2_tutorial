...
> **NOTE:** Instruction templating does a simple `str(value)` substitution. If `{credit_check_result}` resolves to a dict, what the model actually sees in its prompt is that dict's Python `repr`, something like `{'pan_number': 'XYZAB3456C', 'credit_score': 690, ...}`. There's no nested access like `{credit_check_result.credit_score}`, you always get the whole dict as text, and the model reads the field it needs out of that text. This works fine in practice, Claude parses a printed dict without trouble, but it's worth knowing what's actually landing in the prompt.

> **NOTE:** Claude supports combining `output_schema` with `tools` on the same agent, but it's not as rock solid as `output_schema` alone. In rare cases, the model returns plain text instead of the structured fields you asked for, usually a sign it got distracted juggling a tool call and a schema at the same time. If you see this while testing, it's almost always a one-off, rerunning the same input usually fixes it. If it happens consistently on one particular step, that's a sign to simplify that step's instruction rather than something to work around in code.
